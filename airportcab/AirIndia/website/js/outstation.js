...
async function loadOutstationData() {
                ...
                success: function (data) {
                    ...
                    $.each(cityArray, function (i, currProgram) {
                        if (ArrivalCode != '' && currProgram.code == ArrivalCode) {
                            dynamicOption += `<option selected="true" value="${
                                currProgram.code
                            }"> ${
                                currProgram.name
                            } </option>`
    ...
    async function lastDetailsOutstation() {
        ...
        var cab_response = [];
        const departure = await fetch("https://prod.mojoboxx.com/spicescreen/webapi/getCabPartnerData?city="+localStorage["ArrivalStation"]+"&category="+BookingTrip_Type)
    ...
    async function partnerSliderOutstation(cab_response, cabTypeName, cityType) {
        ...
        if (ShowSelfDrive != "yes") {
            if((scv[0] == "QUICKRIDE") ||(scv[0] == "BLUSMART"))
    ...
    //                     "terminalCode": localStorage["cityCODE"].trim() == "DEL" ? "T3" : localStorage["TerminalCode"].split("-")[1],
    ...
    //                     "website_url":"spicescreen_arrival_outstation_url",
                ...
                "clubMember": [
                    {
                        ...
                        "website_url":"airindia_departure_outstation_url",
