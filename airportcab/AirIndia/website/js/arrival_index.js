...
function loadRentalhtml() {
    ...
    <div class="container">
        <div class="rentalHour">
            ...
            <div class="time selectedItem" id="six">
                <label for="eightHours" class="color" id="10" onclick="printHour(id)">8Hrs</label>
                <input type="radio" name="time" id="eightHours">
            </div>
            <div class="time" id="twelve">
                <label for="twelveHours" class="color" id="11" onclick="printHour(id)">12Hrs</label>
    ...
    $("#paymentoptions").append(`<h3>Payment options</h3>
        ...
        <div class="radioBox" style="margin-top: 4%">
            <input id="full" type="radio" style="display: inline;" name="paymentoption" checked />
            <label for="full">
            <p style="display: inline;margin-top: 0%; 
            font-family: 'NetflixSansMedium';">Make full payment now </p>
            </label>
            <span style="margin-left: 6%; font-size: 15px;
            font-family: 'NetflixSansMedium'; margin-top: 0px;" id="fullpay">₹... </span>
        </div>`)
}
...
function loadRidehtml() {
    ...
    $("#BydefaultShow").append(`
                        <div class="drop_div" id="pickupDiv">
                        <select class="drop_ui" id="cabPickupCity" style="margin-left: 11px;">
                        ...
                        <div style="width: 100%; margin: 4px 0px 6px; float: left;" class="pnr_pickup">
                            <div id="makeSerIcon">
                                <label class="full-field">
                                    <input id="pac-input" name="pac-input" class="input_srch" autocomplete="off"
                                        placeholder="Enter Drop location (Min 7 Char)" />
                                    <i class="Locationbox fas fa-search" aria-hidden="true"
                                        id="makeSerIconI"></i>
                                </label>
                            </div>
                            <ul class="autocomplete-results">
                        ...
                        <div class="manualoption" style="display: none;">
                            ...
                            <select id="dubai">
                                <option value="" selected disabled>Update your Drop location</option>
                        ...
                        $("#paymentoptions").empty();
                        $("#paymentoptions").append(`  <h3>Payment options</h3>
                                            <div class="radioBox" style="margin-top: -2%;" id="radioBox1">
                                                <input id="part" type="radio" name="paymentoption" value="PayNow" checked />
                                                <label for="part">
                                                    <p style="margin-top: 0%;font-family: 'NetflixSansMedium';">Advance Payment (Pay Now)
                                                        <span><img src="img/info.png" class="PayInfo" id="PayNowinfo"
                                                                onclick="infoBox('prepaid')" /></span>
                                                        <br>
                                                        <span style="font-weight: lighter;">Price guarantee</span>
                                                    </p>
                                                </label>
                                                <span id="PayNow">₹...</span>
                                            </div>
                        
                                            <div class="radioBox" style="margin-top: -2%;" id="radioBox2">
                                                <input id="full" type="radio" name="paymentoption" value="PayLater" />
                                                <label for="full">
                                                    <p style="margin-top: 0%;font-family: 'NetflixSansMedium';">Pay Later
                                                        <span><img src="img/info.png" class="PayInfo" id="PayLaterinfo"
                                                                onclick="infoBox('paylater')" /></span>
                                                        <br>
                                                        <span style="font-weight: lighter;">Price subject to change</span>
                                                    </p>
                                                </label>
                                                <span id="PayLater">₹...</span>
                                            </div>`)
}
...
// <div class="drop_div" id="pickupDiv2">
//     <select class="drop_ui" id="cabPickupTerminal" style="margin-left:0px">
//         <option selected disabled>Select Drop Terminal</option>
//         </select>
//         <i class="fas fa-sort-down"></i>
// </div>
...
function applyCoupon(){
    ...
    if (localStorage["PNR_Data"] == "Found") {
        let pnrD = JSON.parse(localStorage["pnrData"]);
        Track_analytics(localStorage["booking_id"], pnrD[0]["FirstName"], pnrD[0]["ArrivalStation"], pnrD[0]["DepartureStation"], pnrD[0]["STA"], pnrD[0]["STD"], pnrD[0]["PassengerNumber"], "NULL", "ArrivalcouponcodeApply_PNR");
    } else {
        Track_analytics(localStorage["booking_id"], "C2ACustomer", "NULL", "NULL", "NULL", "NULL", $("#mb_number").val(), "NULL", "ArrivalcouponcodeApply_non-pnr");
    }
    ...
    // if ($("#coupon").val() != '' && ($("#coupon").val() == 'SJ75')) {
    //     $("#CouponCode").val() = $("#coupon").val();
    //     // const rawResponse = await fetch('https://prod.mojoboxx.com/spicescreen/webapi/verifyCouponCode', {
    //         // const rawResponse = await fetch('https://preprod.mojoboxx.com/preprod/webapi/verifyCouponCode', {
    //         const rawResponse = await fetch('https://prodapi.mojoboxx.com/spicescreen/webapi/verifyCouponCode', {
    ...
    //         body: JSON.stringify(
    //             {
    //                 "mobile_no": document.getElementById("mb_number").value,
    ...
    //         $(".popupDiv").css("display", "block")
    //         $(".popupBox").css("display", "block")
    //         $("#coupon").val("Congrats! you'll get a cashback on trip start")
    //         $("#coupon").attr("disabled", "true").css({"width": "95%", "color": "#828282"});
    //         $("#applyCoupon").css("display", "none")
    //         $(".infoBox").css("display", "none")
    //         $("#couponSuccessMsg").css("display", "none")
    //         if (localStorage["PNR_Data"] == "Found") {
    //             let pnrD = JSON.parse(localStorage["pnrData"]);
    //             Track_analytics(localStorage["booking_id"], pnrD[0]["FirstName"], pnrD[0]["ArrivalStation"], pnrD[0]["DepartureStation"], pnrD[0]["STA"], pnrD[0]["STD"], pnrD[0]["PassengerNumber"], "NULL", "RefercodeApply_PNR");
    //         } else {
    //             Track_analytics(localStorage["booking_id"], "C2ACustomer", "NULL", "NULL", "NULL", "NULL", "NULL", "NULL", "RefercodeApply_non-pnr");
    //         }
    //     }else if(document.getElementById("mb_number").value==""){
    //         $("#couponSuccessMsg").css("color", "red").html('Please Enter mobile Number first')
    //     }
    //     else {
...
$('input[type=radio][name=RideType]').change(function () {
    if (this.value == 'Departure') {
        // location.href="../index.html/?pnr="+localStorage["booking_id"] != null ?localStorage["booking_id"]:'null'
