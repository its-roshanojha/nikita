...
// loadCity2City('', 'is_departure');
$("#cabPickupTerminal").empty();
$("#pickup-city2city").val('');
...
localStorage["rideType"] = "citytocity"
...
// await loadCity2City('', 'is_arrival');
initAutocomplete();
LoadDropLocationMap()
getPNR(bookingId);
...
async function loadCity2City(ArrivalCode = '', TripType) {
...
$('#outstationCity').on('change', async function () {
    ...
    if ($("#mb_number").val().length == 10 && $("#mb_number").val() != undefined) {
        await Track_LoadAnalytics(localStorage["mobileNum"], "city2city", "bookcityride", "null", localStorage["OutstationSourceCity"], $(this).find(":selected").val(), localStorage["TerminalCode"], $(this).find(":selected").val(), localStorage["OutstationSourceLat"], localStorage["OutstationSourceLong"], localStorage["DropLatitude"], localStorage["DropLongitude"],
...
document.getElementById("mb_number").onchange = async function () {
    if ($("#mb_number").val().length == 10 && $("#mb_number").val() != undefined) {
        ...
        await Track_LoadAnalytics(localStorage["mobileNum"], "city2city", "bookcityride", "null", localStorage["OutstationSourceCity"], localStorage["cityCODE"], localStorage["TerminalCode"], localStorage["cityCODE"], localStorage["OutstationSourceLat"], localStorage["OutstationSourceLong"], localStorage["DropLatitude"], localStorage["DropLongitude"],
            moment($("#datepicker").val(), ["DD-MM-YYYY", "YYYY-MM-DD", "DD/MM/YYYY"]).format("YYYY-MM-DD"), ($(".timepicker").val() != "Pick up Time" ? $(".timepicker").val() : 'null'))
    }
}
...
//         $("#status").html("Okay");
//         $("#pickup-city2city").val("Not Available");
//         $("#pickup-city2city").prop('disabled', true);
    ...
    geocoder.geocode({
        ...
        if (status == google.maps.GeocoderStatus.OK) {
            if (results[0]) {
                ...
                // console.log(add);
                document.getElementById("pickup-city2city").innerHTML = add;
                document.getElementById("pickup-city2city").value = add;
                var pacInput = document.getElementById("pickup-city2city");
                $("#pickup-city2city").focus();
                const pyrmont = {
                    lat: latitude,
                    lng: longitude
                };
                ...
                function getDistancePrice() {
                    ...
                    // $("#ndl1").html("<b>" + $("#pickup-city2city").val() + "</b>");
                    // $("#ndl2").html("Updated location: <b>" + $("#pickup-city2city").val() + "</b>");
                    ...
                    directionsService.route(route, function (response, status) { // anonymous function to capture directions
                        if (status !== 'OK') {
                            ...
                            if (!directionsData) {
                                ...
                                // }
                                if ($("#mb_number").val().length == 10 && $("#mb_number").val() != undefined) {
                                    Track_LoadAnalytics(localStorage["mobileNum"], "city2city", "bookcityride", "null", localStorage["OutstationSourceCity"], localStorage["cityCODE"], localStorage["TerminalCode"], localStorage["cityCODE"], localStorage["OutstationSourceLat"], localStorage["OutstationSourceLong"], localStorage["DropLatitude"], localStorage["DropLongitude"],
                                        moment($("#datepicker").val(), ["DD-MM-YYYY", "YYYY-MM-DD", "DD/MM/YYYY"]).format("YYYY-MM-DD"), ($(".timepicker").val() != "Pick up Time" ? $(".timepicker").val() : 'null'))
                                }
                ...
                if ($("#pickup-city2city").val() != "") {
                    $("#pickup-city2city").trigger("places_changed");
                }
                $(".Locationbox").removeClass("fa-map-marker-alt");
                $(".Locationbox").addClass("fa-times");
                // initAutocomplete()
...
function initAutocomplete() {
    ...
    let inputContainer = document.querySelector('pickup-city2city');
    ...
    let autocomplete_input = document.getElementById('pickup-city2city');
...
$("#Drop-input").click(function () {
    ...
    if (!localStorage["source_latitudeValue"]) {
        ...
        document.getElementById("pickup-city2city").click();
    }
})

$("#pickup-city2city").click(function () {
    if ($("#outstationCity :selected").text() == "Select Pickup City") {
        $("#ValidateCityMsg").css("display", "block");
        setTimeout(() => {
            $("#ValidateCityMsg").css("display", "none");
...
async function getPNR(bookingId) {
    ...
    PnrdataNotFound();
    if (localStorage["UrlCityCode"] != null && localStorage["UrlCityCode"] != "null") {
        console.log(localStorage["UrlCityCode"]);
        await loadMeruPickPoint(localStorage["UrlCityCode"]);
        await loadCity2City(localStorage["UrlCityCode"], 'is_departure')
        await setLatLong(localStorage["UrlCityCode"]);
        await fillTerminalCodeByCity(localStorage["UrlCityCode"])
        localStorage.setItem("departureAirport", localStorage["UrlCityCode"]);
    }
    Track_analytics(localStorage["booking_id"], "C2ACustomer", "Null", "Null", "Null", "Null", "Null", "NULL", "city2citypageLoad");
}
...
async function PnrdataNotFound() {
    ...
    if ((localStorage["mobileNum"] != "undefined") && (localStorage["mobileNum"] != "") &&( localStorage["mobileNum"] != "null") && ( localStorage["mobileNum"] != null)) {
...
async function lastDetailsOutstation() {
    ...
    var cab_response = [];
    const departure = await fetch(BaseURL+domain+"/webapi/getCabPartnerData?city=" + localStorage["ArrivalStation"] + "&category=city2city")
    const cab_res = await departure.json();
    // console.log(cab_res);
    for (let i in cab_res) {
        if (cab_res[i].platform.toLowerCase() != "host") {
            if (cab_res[i].cab_category.includes('city2city')) {
            ...
            if (cab_city[jk].toLowerCase() == cityType.toLowerCase()) {
                ...
                for (let j = 0; j < clc; j++) {
                    if (cab_Type[j].toLowerCase() == cabTypeName.toLowerCase()) {
                        ...
                        if ((cab_response[i].partner_name == "SAVAARI") ||(cab_response[i].partner_name == "QUICKRIDE") || (cab_response[i].partner_name == "GOZO CABS") || (cab_response[i].partner_name == "BLUSMART")) {
                        ...
                        partner_card.appendChild(distance);
                         $('.coupon_block').css('margin-top', '20%')
...
swiper = new Swiper(".mySwiper", {
    ...
    on: {
        slideChange: async function () {
            ...
            if (ShowSelfDrive != "yes") {
                // if ((currentSliderValue == "QUICKRIDE") || (currentSliderValue == "GOZO CABS") || (currentSliderValue == "BLUSMART")) {
...
async function BookMycab(PAYMENT_TYPE, paytp) {
            ...
            "clubMember": [
                {
                    ...
                    "source": $("#pickup-city2city").val().substring(0, 100).trim(),
                    "destination": localStorage["DropLocationAddress"].trim(),
                    "latitude": localStorage["source_latitudeValue"],
                    "longitude": localStorage["source_longitudeValue"],
                    "isDeparture": 8,
                    ...
                    "source_name": $("#pickup-city2city").val().substring(0, 100),
                    ...
                    "status": "CAB",
                    "mojoPartner": "Yatra",
                    "patner_bookings": sessionStorage["MojoboxxURL_booking"] == "Facebook" ? "Facebook" : "bookairport website",
                    ...
                    "website_url": "yatra_departure_outstation_url",
                    "trip_type": "OneWay",
    ...
    $(".confirmation_boxCabDiv6").css('display', 'block')
    $('#popup_card').empty()
    console.log(dataJ);

    if (dataJ.clubMember[0]['cab_type'].toLowerCase() == "hatchback") {
        $('#popup_card').append("<img src='img/mini_card.png' />")
    }
    else if (dataJ.clubMember[0]['cab_type'].toLowerCase() == "sedan") {
        $('#popup_card').append("<img src='img/sedan_card.png' />")
    }
    else if (dataJ.clubMember[0]['cab_type'].toLowerCase() == "suv") {
        $('#popup_card').append("<img src='img/muv_card.png' />")
    }
    else {
        $('#popup_card').append("<img src='img/mini_card.png' />")
    }
    ...
    document.getElementById('doPayment').onclick = function () {
    ...
    $('.close_popup').click(function () {
        ...
        $('.confirmation_boxCabDiv6').css('display', 'none')
...
async function GetFarefromPartner(PartnercabType) {
    return new Promise(async function (resolve, reject) {
        ...
        var datasend = {
            ...
            destination_latitude: localStorage["DropLatitude"],
            destination_longitude: localStorage["DropLongitude"],
            source_name: $("#pickup-city2city").val(),
            source_city: localStorage["SourceCity"].trim(),
            source_latitude: localStorage["source_latitudeValue"],
            source_longitude: localStorage["source_longitudeValue"],
            ...
            "data": JSON.stringify(
                {
                    ...
                    "source_address": $("#pickup-city2city").val(),
                    "source_latitude": localStorage["source_latitudeValue"],
                    "source_longitude": localStorage["source_longitudeValue"],
...
$("#timeam").click(() => {
    ...
    if ($(".dispTime").hasClass("activeClass")) {
        ...
        if (localStorage["loadPagevalue"] == "citytocity") {
...
function updateTime() {
    ...
    if ((todayDate == localStorage["STDdate"]) || (todayDate == moment($("#datepicker").val(), ["DD-MM-YYYY", "YYYY-MM-DD", "DD/MM/YYYY"]).format("YYYY-MM-DD"))) {
        if (Timevalue < moment().add(15, 'minutes').format('HH:mm')) {
            // $("#cmmsg").html("You have selected an invalid pickup time. You are advised to select a time, 3 hours prior to your departure.");
            $("#cmmsg").html("You have selected an invalid pickup time. You are advised to select a time, 15 minutes later then current Time.");
            $(".thank_msg i").removeClass("fa-check-circle");
            $(".confirmation_boxCabDiv").css("display", "block");
            $(".confirmation_boxCab").css("display", "block");
            $(".timepicker").val("Pick up Time");
            ...
            if ($("#coupon").val() != '') {
                for (let element in data.data) {
                    if ($("#coupon").val().toLowerCase() == data.data[element].coupon_code.toLowerCase()) {
                      ...
                      Track_LoadAnalytics(localStorage["mobileNum"], "couponcodeapply", "city2city", "null", "null", "null", "null",
...
$("#rentalid").click(function(){
  localStorage.setItem("loadPagevalue", "rental")
   });
   
   $("#outstationid").click(function(){
