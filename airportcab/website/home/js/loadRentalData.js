async function loadRentalData() {
    ...
    async function loadMeruPickPoint(CityCode) {
        const meruPickupPoint = await fetch(BaseAPIURL + domain + '/webapi/meruPickupPoint?city=' + CityCode);
    ...
    async function loadRental() {
        var cab_response = [];
        const departure = await fetch(BaseURL + domain + "/webapi/getCabPartnerData?city=" + localStorage["departureAirport"] + "&category=" + BookingTrip_Type)
    ...
    async function partnerSilderRental(cab_response, cabTypeName, cityType) {
                ...
                if (cab_city[jk].toLowerCase() == cityType.toLowerCase()) {
                    ...
                    for (let j = 0; j < clc; j++) {
                        if (cab_Type[j].toLowerCase() == cabTypeName.toLowerCase()) {
                            ...
                            var addCheckLine = document.createElement("div");
                            addCheckLine.setAttribute("id", "addCheckLine")
                            var checkLine = document.createElement("span")
                            checkLine.innerHTML = `Above ${num1}hrs & ${num2}km, additional charges will be applied`

                            addCheckLine.appendChild(checkLine);
                            footerDiv.appendChild(addCheckLine);
        ...
        if (ShowSelfDrive != "yes") {
            if ((scvOutstation[0] == "GOZO CABS") || (scvOutstation[0] == "QUICKRIDE")) {
                $(".auto_btn").css("display", "none");
                $("#ConfirmButton").css("display", "none")
                $(".bookBtn").css("display", "flex")
            ...
            const cashback = await loadCashbackAmt(scvOutstation[0], cityType)
    ...
    async function loadMojoMultiplier(CityCode) {
        ...
        const multiplier = await fetch(BaseAPIURL + domain + '/webapi/mojoboxxMultiplier/?city=' + CityCode + '&time=' + timeUpdate + "&travel_type=rental&platform=BAC")
        // const multiplier = await fetch('https://preprodapi.mojoboxx.com/preprod/webapi/mojoboxxMultiplier/?city=' + CityCode + '&time=' + moment().hour() + "&travel_type=rental&platform=Yatra")
    ...
    swiper = new Swiper(".mySwiper", {
        ...
        on: {
            slideChange: async function () {
                ...
                if (ShowSelfDrive != "yes") {
                    if ((currentSliderValue == "GOZO CABS") || (currentSliderValue == "QUICKRIDE")) {
                    ...
                    const cashback = await loadCashbackAmt(currentSliderValue, localStorage["departureAirport"])
                    const allWithClass = Array.from(
                        document.querySelectorAll('.cashbackAMt')
                    );
                    allWithClass.forEach(element => {
    ...
    async function BookMycab(PAYMENT_TYPE, paytp) {
        ...
        if (ShowSelfDrive == "yes") {
            ...
            dataJ = {
                "clubMember": [
                    {
                        "trip_type": localStorage['tripType'] == 11 ? "PKG_12_120" : "PKG_8_80",
                        "type": "cabForm",
                        ...
                        "website_url": sessionStorage["MojoboxxURL_booking"] == "Facebook" ? "Facebook" : "Yatra_rental_url",
    ...
    async function GetFarefromPartner(PartnercabType) {
        return new Promise(async function (resolve, reject) {
            ...
            var datasend = {
                ...
                end_time: "",
                tripType: "Rental",
                packageId: localStorage['tripType'] == 11 ? "PKG_12_120" : "PKG_8_80",
            }
            $("#fare").css("width", "45%");
            // fetch('https://preprodapi.mojoboxx.com/preprod/webapi/getQuickRideFare', {
    ...
    if (localStorage["loadPagevalue"] == "rental") {
        ...
        function updateTime() {
            ...
            //     $(".thank_msg i").removeClass("fa-check-circle");
            //     $(".confirmation_boxCabDiv").css("display", "block");
            //     $(".confirmation_boxCa if ($("#coupon").val().toLowerCase() == data.data[element].coupon_code.toLowerCase()) {b").css("display", "block");
            //     $(".timepicker").val("Pick up Time");
            // }
        }
    }
