...
var ordernum = "BAC" + Math.floor(10000000000 + Math.random() * 9000000000);
            ...
            $.ajax({
                ...
                success: async function (data) {
                    ...
                    if (data.message == 'Booking Not Found in DB') {
                        ...
                        let { user_name, mobile, email, total_kilometers, pickup_time,
                            isDeparture, source_name,
                            source_city, source_latitude, source_longitude, destination_name, destination_city,
                            destination_latitude, destination_longitude, from_city, terminalCode, user_agent } = data
                        ...
                        userIp = data.user_agent != '' ? data.user_agent : await getuserIp()
                        cityNameFetch = data.destination_city
                        // console.log(loadContent)
                        sessionStorage.setItem('simplOnloadData', JSON.stringify(data))
                        sessionStorage.setItem("BookingPrefillData", JSON.stringify(loadContent))
                        // simplflowLoad()
...
const loadBookingDetails = (async (Bookingdetails) => {
    ...
    async function loadBookingData(Bookingdetails) {
        ...
        $("#currentdate").html(moment((Bookingdetails.pickuptime).split(" ")[0], ["YYYY-MM-DD", 'DD-MM-YYYY']).format('DD-MM-YYYY'));
...
const clickFunction = (async () => {
    ...
    $("#ParsePay_Now").click(function () {
        ...
        addPaymentType('PAYTM', '', '', 'partial_pay', (Number(BookigInnerLoad.service_charge) + Number(149)));
    });
    ...
    $('#fullpaymt').click(function () {
        ...
        $("#flush-disable").val('Pay ₹' + localStorage["TotalFare"])
    })
    $('#fullpaymtl').click(function () {
        // SelectedBookingType = 'PART_PAYTM'
        SelectedBookingType = 'PART_RAZORPAY'
        $("#flush-disable").val('Pay ₹' + 149)


    })
    $('#simplpaymt').click(function () {
        SelectedBookingType = 'SIMPL'
        $("#SimplAmount").css("visibility", "visible");
        $("#simplrow").css("background-color", "#eaf5f1");        
...
// let Userdetails = JSON.parse(sessionStorage.getItem("BookingPrefillData"));
...
window.onload = async () => {
    ...
    if (String(urlParams).includes("bookingid")) {
        ...
        if (String(BookID).includes("success")) {
            if (JSON.parse(decodeURIComponent(urlParams.get('bookingData'))).success == "true") {
                ...
                await simplgetentry(BookingId).then(async () => {
                    ...
                    //     // }).then(async()=>{
                    SelectedBookingType = 'SIMPL';
                    let token = JSON.parse(decodeURIComponent(urlParams.get('bookingData'))).token;
                    await storeSimplToken(token, Userdetails.usermobile).then(async () => {
                        // $('#flush-disable').click();
            ...
            if (JSON.parse(decodeURIComponent(urlParams.get('bookingData'))).success == "false") {
                await simplgetentry(BookingId).then(async () => {
                    $('#toast').modal('show');
                    $("#toastBody").empty();
                    $("#toastBody").html(`The last transaction was unsuccessful.<br> Please retry payment`);
                    $(".pararide").css("display", "none");
...
async function BookSimplCab() {
    ...
    await addPaymentType('SIMPL', '', '', 'full_pay', dataJ.clubMember[0].content_id);
}
...
async function storeSimplToken(token, mobile) {
    const rawResponse = await fetch(
        "https://prodapi.mojoboxx.com/spicescreen/webapi/updateSimplToken",
        {
            ...
            body: JSON.stringify(
                { 'token': token, 'phone_number': mobile }
            )
        }
    );
...
async function loadMainjs(Userdetails) {
    ...
    // UserMobileNumber = Userdetails.usermobile.replace(/^\+(\d{2})/, '');
    UserName = Userdetails.username;
    userEmail = Userdetails.usermail;
    MapPlaceId = Userdetails.MapPlaceId
    KMNum = Userdetails.kilometers.split(' ')[0]
    ...
    //     $(".miniCard").css('display', 'none')
    // }
    if (Userdetails.tripType == 'Ride'){
        $(".ride").css("display", "block");
        $(".rental").css("display", "none");
        } else if (Userdetails.tripType == 'Rental'){
        $(".ride").css("display", "block"); 
    ...
    $(".RentalHour").click(function () {
        ...
        let typ = String($(this).text()).split("HR")[0]
        if (MainCabPartnerName !== 'GOZO CABS'){
            typ == 8 ? RentalHour = 8 : RentalHour = 12; 
        } else{
            typ == 8 ? RentalHour = 10 : RentalHour = 11;          
        } 
        lastDetails(PaymentMethod);
        if (CabBookingType == 'hatchback') {
            $('#sedanFare').empty().html("Click for Fare").removeClass('active-title');
    ...
    async function loadMojoMultiplier(CityCode, Cabtype, kilometer) {
        ...
        const multiplier = await fetch(BaseAPIURL + domain + '/webapi/mojoboxxMultiplier/?city=' + CityCode + '&time=' + timeUpdate + "&travel_type=" + RideType + "&platform=Yatra")
    ...
    async function lastDetails(paymentMethod) {
        var cab_response = [];
        const departure = await fetch(BaseURL + domain + "/webapi/getCabPartnerData?city=" + AirportCode + "&category=" + BookingTrip_Type)
        const cab_res = await departure.json();
        for (let i in cab_res) {
            if (cab_res[i].platform.toLowerCase() != "host") {
                if (cab_res[i].website_name.toLowerCase().includes('yatra')) {
                    if ((BookingTrip_Type == 'Rental') || (BookingTrip_Type == 'Outstation')) {
                        ...
                        if (RideType == 'departure') {
                            if ((cab_res[i].cab_category.includes(BookingTrip_Type)) && (cab_res[i].is_departure == "1")) {
                                cab_response.push(cab_res[i]);
                            }
                        }
                        else {
                            if ((cab_res[i].cab_category.includes(BookingTrip_Type)) && (cab_res[i].isArrival == "1")) {
                                cab_response.push(cab_res[i]);
                            }
                        }
                    }
        ...
        await partnerSlider(cab_response, CabBookingType, Userdetails.citycode, paymentMethod);
    ...
    const loadCashbackAmt = async (partnerName, citycode) => {
        return new Promise((resolve, reject) => {
            ...
            fetch(`${BaseURL}${domain}/webapi/cityPartnerCashback?partner=${partnerName}&city=${citycode}&travel_type=${RideType}&mojoPartner=Yatra`)
                ...
                if (cab_city[jk].toLowerCase() == cityType.toLowerCase()) {
                    ...
                    for (let j = 0; j < clc; j++) {
                        if (cab_Type[j].toLowerCase() == cabTypeName.toLowerCase()) {
                            ...
                            if ((cabpartnername == "QUICKRIDE") || (cabpartnername == "BLUSMART") || (cabpartnername == "MERU") || (cabpartnername == "SAVAARI") || (cabpartnername == "MEGA") || (cabpartnername == "GOZO CABS") || (cabpartnername == "TAXIBAZAAR") || (cabpartnername == "BUDDY CABS") || (cabpartnername == "EEE TAXI") || (cabpartnername == "GOAMILES")) {
    ...
    document.getElementById("flush-disable").onclick = function () {
        if (sessionStorage["PrefillcustomerData"]) {
            let parseData = JSON.parse(sessionStorage["PrefillcustomerData"]);
            // console.log(parseData);
            let usrname = parseData?.name.includes("/") ? parseData?.name.split("/")[0] : parseData.name
    ...
    document.getElementById("nameform").onsubmit = async function (e) {
        ...
        else if (SelectedBookingType == 'SIMPL') {
            $(".spinner").css("display", "block")
            $(".spinnerBack").css("display", "block")
            const eligible = await check_eligibility_fc($("#mb_number").val(), localStorage["TotalFare"])
            if (eligible == 'YES') {
    ...
    async function BookMycab(PAYMENT_TYPE, paytp) {
        ...
        fare_price = parseInt(localStorage["TotalFare"])
        content_id = parseInt(localStorage["TotalFare"])
        ...
        if (MainCabPartnerName == "QUICKRIDE") //// To check quickride fixfareid random issue
        {
        ...
        let serviceCharge = ((MainCabPartnerName == "QUICKRIDE") || (MainCabPartnerName == "GOZO CABS") || (MainCabPartnerName == "BLUSMART") || (MainCabPartnerName == "MERU") || (MainCabPartnerName == "SAVAARI") || (MainCabPartnerName == "MEGA") || (MainCabPartnerName == "BUDDY CABS") || (MainCabPartnerName == "TAXIBAZAAR") || (MainCabPartnerName == "EEE TAXI") || (MainCabPartnerName == "GOAMILES")) ? localStorage["multiplier" + MainCabPartnerName] : 0
        let advanceAmount = ''
        if (SelectedBookingType == 'PART_PAYTM' || SelectedBookingType == 'PART_RAZORPAY') {
            content_id = 149;
            advanceAmount = 149;
        }
        else {
            advanceAmount = ((Number(localStorage["TotalFare"]) - Number(serviceCharge)))
        }
        ...
        Track_LoadAnalytics($("#mb_number").val(), "Yatra", Userdetails.tripType, $("#userName").val(), SourceCity.trim(), AirportCode.trim(), TerminalCode,
        Destination_Name, pickup_lat, pickup_long, destination_latitude, destination_longitude,pickup_time.split(" ")[0], pickup_time.split(" ")[1], localStorage.CouponCode != undefined ? localStorage.CouponCode : '', $("#userEmail").val(), "MOJO_PAGE")
        dataJ = {
            "clubMember": [
                {
                    "type": "cabForm",
                    ...
                    "time": Date.now(),
                    "sendLeadSms": "true",
                    "partnerName": ((localStorage["MojoboxxFare"] == undefined) || (localStorage["MojoboxxFare"] == "undefined") || (localStorage["MojoboxxFare"] == null) || (localStorage["MojoboxxFare"] == "null")) ? MainCabPartnerName : (localStorage["partnercabType"] == 'suv') ? 'QUICKRIDE' : MojoPartnerName,
                    ...
                    "cab_id": MainCabPartnerName == "GOZO CABS" ? GOZOFareId : (MainCabPartnerName == "GOAMILES") ? GoamilesvehicleTyp : 0,
                    "fare_price": fare_price,
                    "total_kilometers": KMNum,
                    "terminalCode": TerminalCode,
                    ...
                    "pnr": '',
                    "source_city": ((AirportCode == 'IXC') && (MainCabPartnerName == "BUDDY CABS")) ? 'Chandigarh' : SourceCity.trim(),
                    ...
                    "fixedFareId": MainCabPartnerName == "QUICKRIDE" ? quickrideFareId : MainCabPartnerName == "MERU" ? localStorage["meruSearchId"] : MainCabPartnerName == "MEGA" ? localStorage["megaSearchId"] : MainCabPartnerName == "GOAMILES" ? Goamilesfareid : "",
                    "mojoPartner": sessionStorage["MojoboxxURL_booking"] == "Mojoboxx" ? "Mojoboxx" : "Yatra",
                    "carID": MainCabPartnerName == "SAVAARI" ? sessionStorage.carID : '',
                    "token": MainCabPartnerName == "SAVAARI" ? sessionStorage.token : '',
                    "website_url": sessionStorage["MojoboxxURL_booking"] == "Facebook" ? "Facebook" : sessionStorage["MojoboxxURL_booking"] == 'MojoboxxI' ? 'bookairportcab.com/MI' : sessionStorage["MojoboxxURL_booking"] == "MojoboxxR" ? "bookairportcab.com/ER" : (sessionStorage["AirportRideType"] == 'arrival' ? 'yatra_Arrival_url' : 'yatra_Departure_url'),
                    "user_agent": '',
                    "pay_type": 'post',
                    'paymentMethod': 'PAYBYCASH',
                    ...
                    'discount_amount': couponcodeType === 'cashback' ? CashbackAmt : defaultCashback
                }
            ]
        };
        console.log(dataJ);
        if (BookingTrip_Type == 'Rental') {
            dataJ.clubMember[0]["trip_type"] = (RentalHour == 12) ? "PKG_12_120" : "PKG_8_80"
            dataJ.clubMember[0]["fare_price"] = (paytp == 'part') ? fare_price : advanceAmount
            dataJ.clubMember[0]["content_id"] = content_id
        }
        if (BookingTrip_Type == 'Outstation') {
        ...
        if (PAYMENT_TYPE == 'SIMPL_eligibility') {
            ...
            if ((dataJ.card_type != 'mojoFixFare') && (MojopartnerReset == 1)) {
                fetch(`${BaseAPIURL}${domain}/webapi/partnerBookingCountReset?isReset=1&cityCode=${localStorage["cityCODE"]}&travelType=${RideType}`)
            }
    ...
    async function loadFareFormDB(partnerName, cityName, distance, cabType) {
        ...
        let actualFareURL = ""
        if (BookingTrip_Type == "Rental") {
            actualFareURL = 'https://prodcaroma.mojoboxx.com/api/v1/prod/thirdparty/getActualFareMotoGp?partner=' + partnerName + '&city=' + cityName + '&km=' + "0" + '&cab_type=' + cabType + '&trip_type=' + "Rental" + '&package=' + RentalHour
        } else {
            actualFareURL = 'https://prodcaroma.mojoboxx.com/api/v1/prod/thirdparty/getActualFare?partner=' + partnerName + '&city=' + cityName + '&km=' + distance + '&cab_type=' + cabType + '&trip_type=' + RideType
        }
        return new Promise(async function (resolve, reject) { // console.log('https://prodcaroma.mojoboxx.com/api/v1/prod/thirdparty/getActualFare?partner=' + partnerName + '&city='+ cityName +'&km=' +distance +'&cab_type=' + cabType +'&trip_type=Departure')
            fetch(actualFareURL, { method: 'GET' }).then(response => response.json()).then(json => { // console.log(json)
                desP = json.data;
                // console.log(json)
    ...
    async function loadPartnerData(cityName) {
        try {
            return new Promise(async function (resolve, reject) {
                fetch(`${BaseAPIURL}${domain}/webapi/mojofixBookingCount?city=${cityName}&type=${RideType}`, {
                    method: 'GET'
                }).then(response => response.json())
                    .then(json => {
                        // console.log(json);
                        if (json.data.length >= 1) {
                            ...
                            for (let k in json.data) {
                                if (json.data[k].travel_type == RideType && (json.data[k].city == cityCODE)) {
    ...
    async function loadFareFromMojoboxx(partnerName, cityName, distance, cabType) {
        ...
        var amt;
        return new Promise(async function (resolve, reject) {
            fetch(BaseAPIURL + domain + '/webapi/mojoboxxfixfare?cab_type=' + cabType + '&km=' + distance + '&city_code=' + cityName + '&travel_type=' + RideType + '&isReset=' + MojopartnerReset, { method: 'GET' }).then(response => response.json()).then(json => { // console.log(json)
    ...
    async function coop_call(PartnercabType) {
        return new Promise(async (resolve, reject) => {
            var suratCity
            if (Destination_city.trim() == "Hazira" || Destination_city.trim() == "Dumas" || Destination_city.trim() == "Rundh" || Destination_city.trim() == "Limla") {
                suratCity = "surat"
        ...
        return new Promise(async function (resolve, reject) {
            ...
            fetch(BaseAPIURL + domain + "/webapi/getBlusmartFare", requestOptions)
                ...
                .then(result => {
                    ...
                    let amountValue = parseInt(newResult.response.car_types[0].fare_details.grand_total) + parseInt(MultiplierAmount);
        ...
        return new Promise(async function (resolve, reject) {
            ...
            if (BookingTrip_Type == 'Rental') {
                datasend.tripType = 'Rental',
                    datasend["packageId"] = (RentalHour == 12) ? "PKG_12_120" : "PKG_8_80"
            }
            if (BookingTrip_Type == 'Outstation') {
                datasend.tripType = 'Outstation',
                    datasend.journeyType = "OneWay"
            }
    ...
    async function filldetailsInCard(partnerName, amount) {
        ...
        //     let discountpercent = parseInt(((amount - MultiplierAmount) * couponCodeValue) / 100)
        //     defaultCashback = discountpercent
        // }
        // let discounted_amount = Math.ceil(Number(amount) - Number(defaultCashback))
        if (CabBookingType == 'hatchback') {
            $("#miniFare").empty();
            $('#miniFare').html("₹ " + amount)
            $('#miniFare').addClass('active-title')
        }
        else if (CabBookingType == 'sedan') {
            $("#sedanFare").empty();
            $('#sedanFare').html("₹ " + amount)
        ...
        if (SelectedBookingType === 'PAYBYCASH') {
            document.getElementById("PayNow").innerHTML = "₹ " + amount
            document.getElementById("SimplAmount").innerHTML = "₹ " + amount
            document.getElementById("PayNow1").innerHTML = "₹ " + amount
            $("#flush-disable").val('Book Now')
        } 
        else if (SelectedBookingType === 'PART_RAZORPAY') {
            document.getElementById("PayNow").innerHTML = "₹ " + amount
        ...
        if (couponcodeType.toLowerCase() == 'discount' || couponcodeType == 'discount%') {
            localStorage.setItem("finalFare" + partnerName, Number(defaultCashback));
            localStorage.setItem("TotalFare", (Number(amount) - Number(defaultCashback)));
            }
            else {
                    ...
                    for (let i of x) {
                        ...
                        if ((i.type == "YATRA_PAYMENT") && (i.website_url == "YATRA")) {
                            ...
                            if (i.city_code.includes(AirportCode)) {
                                ...
                                displayPayment(i.payment_options, i.Pay_btn1, i.Pay_btn2, i.payment_method, i.partial_amount)
                                z = 1;
                            }
                        }
                    }
    ...
    async function displayPayment(paymentType, FullType, PartialType, RideType, partialAmount) {
        ...
        if (RideType.toLowerCase().includes(pageType.toLowerCase())) {
            if (paymentType.toLowerCase() == 'cash,partial' || paymentType.toLowerCase() == 'partial,cash') {
            ...
            if (paymentType.toLowerCase() == 'full,partial' || paymentType.toLowerCase() == 'partial,full') {
            ...
            if (paymentType.toLowerCase() == 'full,cash,partial' || paymentType.toLowerCase() == 'full,partial,cash'
                || paymentType.toLowerCase() == 'cash,full,partial' || paymentType.toLowerCase() == 'cash,partial,full'
                || paymentType.toLowerCase() == 'partial,full,cash' || paymentType.toLowerCase() == 'partial,cash,full') {
                $("#fullbutton").css("display", "block")
                $("#laterbutton").css("display", "block")
                $("#partbutton").css("display", "block")
                $(".paymentoptionnew").css("justify-content", "space-around")
            ...
            if (paymentType.toLowerCase() == 'full,cash' || paymentType.toLowerCase() == 'cash,full') {
                $("#fullbutton").css("display", "block")
                $("#laterbutton").css("display", "block")
                $(".paymentoptionnew").css("justify-content", "space-around")
                sessionStorage.setItem("fullPayType", FullType)
                        ...
                        if (data.success === true) {
                            for (let element in data.data) {
                                if ($("#coupon").val().toLowerCase() == data.data[element].coupon_code.toLowerCase()) {
                                    couponapplied = true;
                                    console.log(data)
                                    Track_LoadAnalytics(localStorage["mobileNum"], "couponcodeapplydeparture", "Yatra", "null", "null", "null", "null",
...
async function loadPaymentMethod(paytype, paymentMethod) {
    ...
    let firstname = String(paymentMethod).includes(',') ? paymentMethod.split(',')[0] : paymentMethod
    ...
    if (paytype == 'full') {
        document.getElementById("PayNow").innerHTML = "₹ " + localStorage["TotalFare"]
        document.getElementById("RazorPayAmount").innerHTML = "₹ " + localStorage["TotalFare"]
        document.getElementById("SimplAmount").innerHTML = "₹ " + localStorage["TotalFare"]
        let val = $("input[name='fav_language']:checked").val().toUpperCase()
        SelectedBookingType = `FULL_${val}`
    }
    if (paytype == 'partial') {
        document.getElementById("PayNow").innerHTML = "₹ " + sessionStorage["partialAMount"]
        document.getElementById("RazorPayAmount").innerHTML = "₹ " + sessionStorage["partialAMount"]
        document.getElementById("SimplAmount").innerHTML = "₹ " + sessionStorage["partialAMount"]
        let val = $("input[name='fav_language']:checked").val().toUpperCase()
    ...
    if (userAgent.includes("Android")) {
        ...
        if (iOSInfo && iOSInfo.length > 1) {
            ...
            for (const part of parts) {
                if (part.includes("iPhone") || part.includes("iPad") || part.includes("iPod")) {
                    model = part.trim();
                    manufacturer = "Apple";
                    break;
                }
...
const check_eligibility_onload = async (userMobile, FareAMOUNT) => {
    ...
    else if (content.data.data.status == "eligible" && content.data.data.error_code == null && content.data.data.redirection_url == null) {
        // $("#simplrow").css("display", "flex");
        // $("#payline2").css("display", "block");
        simpleligiblityType = 'eligible';
    }
    else if (content.data.data.status == "eligible" && content.data.data.error_code == "linking_required") {
        // $("#simplrow").css("display", "flex");
        // $("#payline2").css("display", "block");
        simpleligiblityType = 'eligible';
    }
...
const check_eligibility_fc = async (userMobile, FareAMOUNT) => {
    ...
    if (content.data.data.error_code == "pending_dues") {
        $(".simplStatus").html(
            `Pending Bill Rs. ${content.data.data.metadata.pending_due_in_paise / 100}`).css("display", "block")
        $("#flush-disable").val('Pay Now')
        SimplButon = 'Redirect'
        $("#exampleModal").modal("hide");
        $('#toast').modal('show');
        $("#toastBody").empty();
        $("#toastBody").html(`You have Pending dues Rs. ${content.data.data.metadata.pending_due_in_paise / 100}`);
        $(".spinner").css("display", "none")
        $(".spinnerBack").css("display", "none")
        ...
        $(".simplStatus").html(
            `Insufficient Credit Rs. ${content.data.data.metadata.pending_due_in_paise / 100}`).css("display", "block")
        $("#flush-disable").val('Pay Now')
        SimplButon = 'Redirect'
        $("#exampleModal").modal("hide");
        $('#toast').modal('show');
        $("#toastBody").empty();
        $("#toastBody").html(`Insufficient Credit Rs. ${content.data.data.metadata.pending_due_in_paise / 100}`);
        $(".spinner").css("display", "none")
        $(".spinnerBack").css("display", "none")
        ...
        $(".spinner").css("display", "none")
        $(".spinnerBack").css("display", "none")
        simplRedirectionURL = `${content.data.data.redirection_url}&merchant_payload=BOOKAIRPORTCAB-000-linking_required-${userMobile}-000-accountlinked=true-bookingid-${ordernum}-amount-${FareAMOUNT}-callbackURL-https://yatra.bookairportcab.com/website/home/payNow.html`;
        return 'YES';
    }
        ...
        $("#flush-disable").val('Pay Now')
        $(".simplStatus").html(
            `Your credit limit is Rs. ${content.data.data.available_credit_in_paise / 100
            }`
        ).css("display", "block")
        $(".spinner").css("display", "none")
        $(".spinnerBack").css("display", "none")
