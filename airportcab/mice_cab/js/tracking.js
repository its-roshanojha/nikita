...
function Track_analytics(PNR, Name, Dest, source, STA, STD, mobile, OTP, title) {
    dataJ = {
        "clubMember": [{
            "type": sessionStorage["MojoboxxURL_booking"] == "Mojoboxx"?"BAC_mojoboxx Analytics":sessionStorage["MojoboxxURL_booking"] == "Facebook"?"Facebook": localStorage["TravelType"] == "International" ? "Yatra_International Analytics" : "Yatra Analytics",
            "user_name": Name,
            "last_name": "Customer",
            "mobile": mobile,
            ...
            "time": Date.now(),
            "sendLeadSms": "true",
            "title": sessionStorage["MojoboxxURL_booking"] == "Mojoboxx"?"BAC_mojoboxx_departure":sessionStorage["MojoboxxURL_booking"] == "Facebook"?"BAC_Facebook":title,
            ...
            "OTP": OTP,
            "pnr": PNR,
            "status": sessionStorage["MojoboxxURL_booking"] == "Mojoboxx"?"BAC_mojoboxx_departure":sessionStorage["MojoboxxURL_booking"] == "Facebook"?"BAC_Facebook":title,
        }]
    };
...
async function Track_LoadAnalytics(mobile, pageName, title, Name, source_city, citycode, Terminal, Dest_city,
   source_lat, source_long, dest_lat, dest_long, pickup_date, pickuptime,coupon_code='',email='',partner='', KMNum='', content_id= '') {
  console.log(msgUniqueIdvalue)
  ...
  dataJ = {
          ...
          "pagename": pageName,
          "title": title,
          "type": sessionStorage["MojoboxxURL_booking"] == "Mojoboxx"?"BAC_mojoboxx page load":sessionStorage["MojoboxxURL_booking"] == "Facebook"?"Facebook": localStorage["TravelType"] == "International"? "Yatra_International page load" : "Yatra page load",
          "user_name": Name,
          "last_name": "Customer",
